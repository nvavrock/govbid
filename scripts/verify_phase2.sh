#!/usr/bin/env bash
# Verify Phase 2: dashboard UI, review actions, Slack digest script.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"

FAIL=0
warn() { echo "WARN $*"; }
fail() { echo "FAIL $*"; FAIL=1; }
ok() { echo "OK   $*"; }

echo "=== Phase 2 verification ==="

if ! bash "$ROOT/scripts/verify_phase1.sh" >/dev/null 2>&1; then
  fail "Phase 1 verification failed — fix Phase 1 first"
else
  ok "Phase 1 checks passed"
fi

if [[ -f .env ]]; then
  set -a
  # shellcheck disable=SC1091
  source .env
  set +a
fi

UV_BIN="$(command -v uv 2>/dev/null || true)"
if [[ -z "$UV_BIN" ]]; then
  fail "uv not found"
else
  if "$UV_BIN" run python -c "import streamlit; import consig.ui" 2>/dev/null; then
    ok "Consig UI module loads (streamlit)"
  else
    fail "Consig UI import failed"
  fi

  if "$UV_BIN" run scripts/send_review_digest.py --dry-run >/dev/null 2>&1; then
    ok "send_review_digest.py --dry-run"
  else
    fail "send_review_digest.py --dry-run failed"
  fi
fi

# shellcheck source=lib/docker.sh
source "$ROOT/scripts/lib/docker.sh" 2>/dev/null || true
if command -v docker >/dev/null 2>&1 && govbid_resolve_docker >/dev/null 2>&1; then
  shortlist_count="$(govbid_docker_compose exec -T postgres psql -U "${POSTGRES_USER:-govbid}" -d "${POSTGRES_DB:-govbid}" -tAc \
    "SELECT COUNT(*) FROM match_scores WHERE review_status IN ('reviewing', 'bid');" 2>/dev/null | tr -d '[:space:]' || echo 0)"
  if [[ "${shortlist_count:-0}" -ge 1 ]]; then
    ok "shortlist has ${shortlist_count} row(s) (reviewing/bid)"
  else
    warn "no reviewing/bid rows yet — use Consig UI Shortlist/Bid buttons"
  fi
else
  warn "Docker not available — skipped shortlist DB check"
fi

if [[ -n "${SLACK_WEBHOOK_URL:-}" ]]; then
  ok "SLACK_WEBHOOK_URL is set"
else
  warn "SLACK_WEBHOOK_URL not set — digest --send will fail until configured"
fi

echo ""
if [[ "$FAIL" -eq 0 ]]; then
  echo "Phase 2 verification PASSED"
  exit 0
fi
echo "Phase 2 verification FAILED"
exit 1
