...
check "transcripts directory" test -d "$PROJECT/transcripts/govclose"
...
check "yt-dlp available" bash -c 'command -v yt-dlp >/dev/null'
check "download script syntax" python3 -m py_compile "$PROJECT/scripts/download_sam_opportunities.py"
check "fetch script syntax" bash -n "$PROJECT/scripts/fetch_govclose_transcripts.sh"
check "run_download.sh syntax" bash -n "$PROJECT/run_download.sh"
check "run_govclose_transcripts.sh syntax" bash -n "$PROJECT/run_govclose_transcripts.sh"
